...
class MysqlPipeline(object):
    ...
    def __init__(self):
        #链接数据库
        self.conn = MySQLdb.connect('localhost','root','liyuanhao9286A','lyh',charset='utf8',use_unicode = True)#mysql使用的是gbk，而python使用的是ut8，所以鼻血设定后两个参数，后一个参数为是否使用字符集
        #创建数据库浮标
...
class MysqlTwistedPipline(object):
    ...
    def from_settings(cls,settings):#调用settings值的固定函数，调用方法和字典一样
        ...
        dbpool = adbapi.ConnectionPool('MySQLdb',**dbparms)#第一个是需要的函数名称，后边是不定长的字典参数
